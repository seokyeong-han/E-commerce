...
public interface PointApi {
    //Swagger 문서화 기준 (계약 정의)

    @Operation(summary = "유저 포인트 검색", description = "유저 포인트 API")
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "조회 성공",
    ...
    @Operation(summary = "포인트 충전", description = "유저 포인트를 충전합니다.")
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "충전 성공",
    ...
    CommApiResponse<PointChargeResponse> charge(@RequestBody PointChargeRequest request);
}
